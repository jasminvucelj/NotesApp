...
public class PseudoClusterList {
    private List<PseudoCluster> clusterList;

    private double distanceThreshold;


    PseudoClusterList(double distanceThreshold) {
    ...
    int getTotalElementCount() {
        ...
        if (size == 0) {
            ...
            int sum = 0;
            for (PseudoCluster cluster : clusterList) {
                sum += cluster.size();
            }
            return sum;
        }
    ...
    private void addCluster(Location location) {
        PseudoCluster element = new PseudoCluster(location);
     ...
     */
    PseudoCluster getLargestCluster() {
        int size = clusterList.size();
        if (size == 0) {
            return null;
        }
